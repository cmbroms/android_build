...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cmb_") ; then
       CMB_BUILD=$(echo -n $1 | sed -e 's/^cmb_//g')
    else
       CMB_BUILD=
    fi
    export CMB_BUILD
...
{
    target=$1
    CMB_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/cmb/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CMB_VERSION)
        ZIPFILE=cmb-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cmb.device=$CMB_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CMB_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CMB_FIXUP_COMMON_OUT ]; then
